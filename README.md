<table width="100%">
  <tr>
    <td>
      <strong>APP :</strong> <a href="https://band-diary.vercel.app">Bandiary</a><br>
      <strong>TEST ACCOUNT :</strong> test / 1234
    </td>
    <td align="right">
      <img width="120" height="120" alt="Bandiary QR Code" src="https://github.com/user-attachments/assets/eb46ad2e-ff4d-4bb9-a73a-e70b89236276">
    </td>
  </tr>
</table>

<img width="800" height="450" alt="인트로" src="https://github.com/user-attachments/assets/b2a8f441-3a84-4422-bda7-d83d2db18117">

# 🎸 Bandiary

> Current Version: 'v.0.4.2'

밴드별 활동을 기록하고 관리할 수 있는 모바일 중심 웹앱입니다. ( 테스트 시 모바일 환경을 권장합니다. )
사용자는 자신의 밴드 프로필을 설정하고,</br>
사진, 영상, 오디오를 업로드하여 활동 기록을 남기고 합주 및 개인 영상에 대한 피드백을 할 수 있습니다.<br />
또한 합주실, 식당 등의 장소 정보를 관리하고, 세션별 PDF 악보를 등록해 연습 자료로 활용할 수 있습니다.<br />
밴드별 공지사항 및 회의 메모 정보 등을 저장할 수 있습니다.

## 📌 프로젝트 소개

`Bandiary`는 Band + Diary의 의미를 담은 밴드 활동 기록 서비스입니다.
밴드 멤버가 공연 사진, 합주 영상, 악보 PDF, 연습 장소 정보를 한 곳에서 관리할 수 있도록 제작했습니다.

모바일 환경에서 앱처럼 사용할 수 있도록 PWA 적용을 고려했으며, Supabase를 활용해 별도의 백엔드 서버 없이 데이터베이스와 파일 스토리지를 연동했습니다.

## ✨ 주요 기능

### 1. 로그인

* Supabase 기반 로그인
* 신규 계정 발급은 로그인 화면의 `Signup` 링크를 통해 관리자에게 문의

### 2. 프로필 관리

* 프로필 이미지, 설명, 밴드원 수정

### 3. 콘텐츠 관리

* Supabase Storage에 사진 / 비디오 / 오디오 파일 / 유튜브 콘텐츠 업로드
* 전체보기 / 비디오 / 사진 / 오디오 / 유튜브 필터링 (기본 최신 등록순 카드 정렬)
* 상세 모달에서 이미지, 영상, 오디오 파일 확인
* 콘텐츠별 피드백, 구간 반복 기능

### 4. 장소 관리

* 스튜디오 / 식당 장소 정보 등록
* 카카오맵 API를 이용한 지도 표시
* 카카오맵 API를 이용한 장소 검색 및 등록
* 카카오맵 길찾기 연결

### 5. 악보 관리

* PDF 악보 업로드
* Vocal, Guitar, Bass, Keyboard, Drum 세션별 필터링
* PDF 미리보기 및 다운로드

### 6. 일정 관리

* FullCalendar 라이브러리 사용하여 일정 관리
* 밴드 일정 · 공연 일정 · 회의 일정 등을 추가·삭제

### 7. 공지사항 및 메모 관리

* 밴드 공지사항 및 메모 관리
* 중요 공지, 전체 목록 분리 후 필터링

## 🛠 기술 스택

### Frontend

* React
* JavaScript
* React Router
* Vite

### Backend / Database

* Supabase Database
* Supabase Storage

### Library

* react-pdf
* pdfjs-dist
* Kakao Map API
* FullCalendar
* FFmpeg

### Deploy

* Vercel

### AI

* Codex 프로젝트 적용 테스트 예정

### Function Test

* Microsoft Clarity

## 📦 Version History

### v0.5.0 - 2026.09.02 ~ 업데이트중
* 메인페이지 유튜브 콘텐츠 상세 모달 추가
* 밴드별 데이터 분리 처리 추가
* 장소 관리 페이지 추천 장소 기능 추가

### v0.4.0 - 2026.08.14 ~ 2026.09.01
* 악보 관리 페이지 리뉴얼
* 장소 관리 페이지 리뉴얼
* 일정 관리 페이지 리뉴얼
* 사진, 비디오, 오디오별 콘텐츠 상세 모달 분리 후 리뉴얼

### v0.3.0 - 2026.08.01 ~ 2026.08.13
* 밴드 공지사항 및 메모 기능 페이지 추가
* 메인페이지 레이아웃 UI / UX 개선
* 프로필 수정 모달 UI / UX 개선
* 콘텐츠 모달 UI / UX 개선

### v0.2.0 - 2026.06 ~ 2026.07
* 사진 / 비디오 / 오디오 업로드 기능 메인 페이지 추가 ( Supabase Storage 연동 )
* 일정 관리 기능 페이지 추가 ( FullCalendar 적용 )
* 콘텐츠 모달 UI / UX 개선

### v0.1.0 - 2026.05
* 프로젝트 초기 세팅
* 로그인 페이지 추가
* 프로필 관리 모달 추가
* 장소 및 악보 관리 페이지 추가

## ✅ 향후 개선 사항

* Supabase Auth 기반 로그인 구조로 변경
* 관리자 권한을 프론트엔드가 아닌 Supabase RLS 정책으로 관리
* 밴드별 콘텐츠 분리 조회 기능 추가
